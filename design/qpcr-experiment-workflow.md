...
- **Target and fluorophore/reporter**: Users specify **Target Name** and **Fluorophore** (Bio-Rad) or **Reporter/Quencher** (Quant Studio) for the assay. These can be set on the experiment or passed when downloading the template; default fluorophore/reporter (e.g. FAM) is used when not specified. See [qpcr-fluorophore-reporter.md](qpcr-fluorophore-reporter.md) for platform differences (Bio-Rad = channel only; QuantStudio = Reporter + Quencher pair, Quencher = None for SYBR) and dye name mapping (e.g. HEX vs VIC).
...
- **Template output**: Only well positions that have a tube (a record in the plate layout) appear in the template file; positions with no tube are omitted from the output.
- **Plate layout locked after template/results**: Once the experiment status is beyond `setup` (e.g. template exported or results uploaded), plate upload is rejected (409) so results stay aligned with the layout. To change the plate, the user must delete the experiment and create a new one.
