...
When you're importing specimens with containers, the requirements become more specific. You can include an optional `comment` column; values are stored on each container and are useful for notes (e.g. quality, handling). For Micronix tubes, use `plate_name`, a unique barcode for each tube, and a position in the plate. The position must be in the correct format: a letter (A-H) followed by two digits (01-12), like "A01" or "B12". For Cryovial tubes, use `box_name` and a position. Barcodes are optional for cryovial tubes. For Papers, use `bag_name` and a label identifier. For Static Wells, use `plate_name` and a position, using the same A01-H12 format as Micronix tubes. You can also use a single column named `collection_name` instead of `plate_name`, `box_name`, or `bag_name`; it is accepted as an alias for the collection identifier for any container type.
...
- **Missing required columns**  
  The system will tell you exactly which columns are needed. Add them to your CSV and ensure headers match the expected names (case-insensitive). For the collection identifier (plate/box/bag), you can use either the type-specific column (`plate_name`, `box_name`, or `bag_name`) or a single column named `collection_name` for any container type.
