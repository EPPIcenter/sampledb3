...
import { getDefaultStateAndUnit } from './defaults'
import {
  resolveCollection,
...
/**
 * Validate aliquot data based on container type
 */
export async function validateAliquotData(
  containerType: ContainerType,
  data: AliquotData
...
async function createMicronixTube(
  ...
  try {
    ...
    const defaults = await getDefaultStateAndUnit()
    if (!defaults) return { success: false, error: 'No default state/unit found' }

    const now = new Date().toISOString()
    const [container] = await db.insert(storageContainer).values({
      specimenId,
      stateId: data.stateId || defaults.stateId,
      unitId: data.unitId || defaults.unitId,
      totalQuantity: data.totalQuantity ?? 1.0,
      remainingQuantity: data.remainingQuantity ?? data.totalQuantity ?? 1.0,
...
async function createPaper(
  ...
  try {
    ...
    const [container] = await db.insert(storageContainer).values({
      ...
      stateId: data.stateId || defaults.stateId,
      unitId: data.unitId || defaults.unitId,
      totalQuantity: data.totalQuantity ?? 5.0, // DBS defaults to 5 spots
      remainingQuantity: data.remainingQuantity ?? data.totalQuantity ?? 5.0,
      comment: data.comment,
      created: now,
      lastUpdated: now,
    }).returning()
...
/**
 * Main function to create aliquot for specimen
 */
export async function createAliquotForSpecimen(
  specimenId: number,
  data: AliquotData
): Promise<{ success: boolean; containerId?: number; error?: string }> {
  const validation = await validateAliquotData(data.containerType, data)
