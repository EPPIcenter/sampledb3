...
import { storageContainer, specimen, location, state, unit, micronixTube, cryovialTube, micronixPlate, cryovialBox, studySubject, study, specimenType, controlBatch, controlDefinition, tube, box, paper, sheet, bag, staticWell } from '../db/schema'
...
async function enrichContainerDetailed(container: any) {
  ...
  // Get state and unit
  const [containerState, containerUnit] = await Promise.all([
  ...
  const [micronixInfo, cryovialInfo, tubeInfo, paperInfo, staticWellInfo] = await Promise.all([
    ...
    }).from(micronixTube).leftJoin(micronixPlate, eq(micronixTube.manifestId, micronixPlate.id)).where(eq(micronixTube.id, id)).get(),
    ...
    }).from(cryovialTube).leftJoin(cryovialBox, eq(cryovialTube.manifestId, cryovialBox.id)).where(eq(cryovialTube.id, id)).get(),
    ...
    db.select({
      ...
      plateName: micronixPlate.name,
      locationId: micronixPlate.locationId,
    }).from(staticWell).leftJoin(micronixPlate, eq(staticWell.manifestId, micronixPlate.id)).where(eq(staticWell.id, id)).get(),
  ])
  ...
  if (micronixInfo) {
    ...
    manifestInfo = { type: 'micronix_plate', id: micronixInfo.plateId, name: micronixInfo.plateName, position: micronixInfo.position, barcode: micronixInfo.barcode }
  } else if (cryovialInfo) {
    containerType = 'cryovial_tube'
    locationId = cryovialInfo.locationId
    manifestInfo = { type: 'cryovial_box', id: cryovialInfo.boxId, name: cryovialInfo.boxName, position: cryovialInfo.position, barcode: cryovialInfo.barcode }
  } else if (tubeInfo) {
    containerType = 'tube'
    locationId = tubeInfo.locationId
    manifestInfo = { type: 'box', id: tubeInfo.boxId, name: tubeInfo.boxName, position: tubeInfo.boxPosition, label: tubeInfo.label }
  } else if (paperInfo) {
    containerType = 'paper'
    manifestInfo = { type: 'sheet', id: paperInfo.sheetId, name: paperInfo.sheetName, position: paperInfo.position, barcode: paperInfo.barcode }
    ...
    manifestInfo = { type: 'micronix_plate', id: staticWellInfo.plateId, name: staticWellInfo.plateName, position: staticWellInfo.position }
  }
  ...
  return {
    ...
    // Keep legacy fields for compatibility if needed, but 'manifest' is cleaner
...
containers.get('/', async (c) => {
  try {
    ...
    // Enrich containers with location and state info
...
containers.get('/:id', async (c) => {
  try {
    ...
    return c.json({
      ...
      specimen: spec,
      source: sourceInfo,
      // For backward compatibility or if the frontend expects these at top level
      ...enriched,
    })
  } catch (error: any) {
