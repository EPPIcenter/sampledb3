...
export default function ContainerMoveGenericTubes() {
  ...
  const parseFilename = (filename: string): string[] => {
    ...
    const skipWords = ['move', 'moves', 'swap', 'swaps', 'csv', 'tube', 'tubes', 'box']
  ...
  const handleValidateAndResolve = async () => {
    ...
    try {
      ...
      if (types.size > 1 || (types.size === 1 && !types.has('box'))) {
        setValidationErrors([{
          row: 0,
          error: `Mixed or invalid collection types found: ${Array.from(types).join(', ')}. All containers must be from box collections.`,
        }])
        setLoading(false)
        return
      }
