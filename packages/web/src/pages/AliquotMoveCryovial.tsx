...
export default function AliquotMoveCryovial() {
  ...
  const parseFilename = (filename: string): string[] => {
    ...
    const skipWords = ['move', 'moves', 'swap', 'swaps', 'aliquot', 'aliquots', 'csv', 'cryovial', 'box']
  ...
  const handleValidateAndResolve = async () => {
    ...
    try {
      ...
      if (types.size > 1 || (types.size === 1 && !types.has('cryovial_box'))) {
        setValidationErrors([{
          row: 0,
          error: `Mixed or invalid collection types found: ${Array.from(types).join(', ')}. All aliquots must be from cryovial_box collections.`,
        }])
        setLoading(false)
        return
      }
