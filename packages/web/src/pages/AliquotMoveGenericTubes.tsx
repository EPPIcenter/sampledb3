...
export default function AliquotMoveGenericTubes() {
  ...
  const parseFilename = (filename: string): string[] => {
    ...
    const skipWords = ['move', 'moves', 'swap', 'swaps', 'aliquot', 'aliquots', 'csv', 'tube', 'tubes', 'box']
...
BOX-002,1,10`
    ...
    a.href = url
    a.download = 'generic_tubes_move_template.csv'
    a.click()
  ...
  const handleValidateAndResolve = async () => {
    ...
    try {
      ...
      // Build identifiers (position-based for generic tubes)
      ...
      // Verify all are generic boxes
      const types = new Set(collections.map(c => c.type))
      if (types.size > 1 || (types.size === 1 && !types.has('box'))) {
        setValidationErrors([{
          row: 0,
          error: `Mixed or invalid collection types found: ${Array.from(types).join(', ')}. All aliquots must be from box collections.`,
        }])
        setLoading(false)
        return
      }
  ...
  useEffect(() => {
    if (currentStep === 'mapping') {
      collectionsApi.listCollectionsByType('box').then((response) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold mb-6">Move Generic Tubes</h1>
          ...
          <>
            <div className="bg-white rounded-lg shadow p-6 mb-6">
              ...
              <ul className="list-disc list-inside space-y-2 text-gray-700">
                <li>Upload a CSV file with generic tube move operations</li>
                <li>
                ...
                <li>Generic tubes are identified by position (boxPosition)</li>
                <li>Collection type will be inferred (must be box)</li>
        ...
        {currentStep === 'resolve' && (
          <>
            <div className="bg-white rounded-lg shadow p-6 mb-6">
              <h2 className="text-xl font-semibold mb-4">Resolved Generic Tubes</h2>
              <div className="mb-4">
                <p className="text-gray-700">
                  <strong>Collection Type:</strong> box
                </p>
                <p className="text-gray-700">
