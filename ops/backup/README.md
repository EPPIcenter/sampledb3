...
- **`backup-db-restic.sh`** — SQLite online `.backup` to a temp file under `$TMPDIR`, then `restic backup --stdin` (stable restore path `sampledb.sqlite`). If `RESTIC_COPY_REPOSITORY` is set, `restic copy` to that second repo afterward.
